...
export class DeepSeekCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  async provideInlineCompletionItems(
    ...
    log(`[#${reqId}] Using endpoint=${this.config.endpoint} model=${this.config.model} keyLen=${apiKey.length} keyPrefix=${apiKey.substring(0, 6)}...`);
  ...
  private async getApiKey(): Promise<string | undefined> {
    ...
    this.apiKeyPromise = this.loadApiKey();
    return this.apiKeyPromise;
  }

  private async loadApiKey(): Promise<string | undefined> {
    // 优先使用用户在 VS Code 设置中填写的 Key
