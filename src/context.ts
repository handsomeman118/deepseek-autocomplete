...
export function gatherContext(
  ...
  // 提取 imports
  const importEndLine = Math.min(document.lineCount, 30);
  const importEnd = document.lineAt(importEndLine - 1).rangeIncludingLineBreak.end;
  const importText = document.getText(new vscode.Range(new vscode.Position(0, 0), importEnd));
  const imports = extractImports(importText);
